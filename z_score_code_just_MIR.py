...
#input is zip(epoch_flux, epoch_measurements)
def remove_outliers_epochs(input):
    flux = np.array([tup[0] for tup in input])
    ...
    return modified_deviation.tolist(), [tup[1] for tup in input] #return list of epoch modified deviation values and corresponding number of measurements in those epochs
...
for object_name in object_names:
    ...
    W1_mod_dev, W1_epoch_measurements = remove_outliers_epochs(zip(W1_averages_flux, W1_epoch_meas))
    W2_mod_dev, W2_epoch_measurements = remove_outliers_epochs(zip(W2_averages_flux, W2_epoch_meas))

    quantifying_change_data = {
        "W1 modified deviation": W1_mod_dev, #0
        "W1 epoch measurements": W1_epoch_measurements, #1
        "W2 modified deviation": W1_mod_dev, #2
        "W2 epoch measurements": W1_epoch_measurements, #3
    }
    ...
    if my_object == 0:
        df.to_csv(f"AGN_modified_deviation_epoch_measurements_sample_{my_sample}.csv", index=False)
    elif my_object == 1:
        df.to_csv("CLAGN_modified_deviation_epoch_measurements.csv", index=False)
